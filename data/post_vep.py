...
def passes_qc(variant, source: str) -> bool:
    if source != "sg10k": return True
    if variant.INFO.get("IMP") is None: return True
...
def parse_vcf(vcf_path: str, source: str,
    ...
    for v in VCF(vcf_path):
        if chromosomes and v.CHROM not in chromosomes: continue
        if len(v.ALT) > 1: continue
        n_total += 1
        if not passes_qc(v, source): continue
        ac, an, af = get_af(v, source)
        raw = v.INFO.get("CSQ")
        if not raw: continue
        for tx_str in (raw.split(",") if isinstance(raw,str) else list(raw)):
            tx = _parse_csq(tx_str, source)
            if not tx["MANE_SELECT"].strip(): continue
            if "missense_variant" not in tx["Consequence"]: continue
            if tx["BIOTYPE"] != "protein_coding": continue
            ref_aa, alt_aa = _parse_aa(tx["Amino_acids"], tx["HGVSp"])
            if not ref_aa or not alt_aa or ref_aa==alt_aa: continue
            if "*" in (ref_aa, alt_aa): continue
            try: ppos = int(tx["Protein_position"].split("-")[0])
            except: continue
            n_kept += 1
            records.append({
...
def maf_weight(af: Optional[float], source: str) -> float:
    if source == "indigen": return 0.2
    if af is None or (isinstance(af,float) and np.isnan(af)): return 0.2
    for lo,hi,w in [(2e-4,1e9,1.0),(7.37e-5,2e-4,0.8),
                    (2.71e-5,7.37e-5,0.4),(1e-5,2.71e-5,0.2),(0,1e-5,0.1)]:
        if lo <= af < hi: return w
    return 1.0
        ...
        if source == "indigen" and not indigen_done:
            ...
            for chrom in chromosomes:
                ...
                if not p:
                    ...
                    continue
                try: src_frames.append(parse_vcf(p, source, [chrom]))
                except Exception as e: log.error("%s %s: %s", source, chrom, e)
        if src_frames:
